---
title: "What is Airflow®? — Airflow Documentation"
...
This examples uses a simple Bash command and Python function, but Airflow tasks can run virtually any code. You might use tasks to run a Spark job, move files between storage buckets, or send a notification email. Here’s what that same dag looks like over time, with multiple runs:
...
![Overview of a complex DAG in the Grid View, showing the status of all DAG runs, as well as quick links to recently failed task logs](https://airflow.apache.org/docs/apache-airflow/stable/_images/demo_dag_overview_with_failed_tasks.png)
...
The Airflow platform is highly customizable. With the [Public Interface of Airflow](https://airflow.apache.org/docs/apache-airflow/stable/public-airflow-interface.html) you can extend and adapt nearly every part of the system — from operators to UI plugins to execution logic.
